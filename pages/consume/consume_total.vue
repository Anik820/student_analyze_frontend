    ...
    <Content :style="{padding: '0 50px'}">
      ...
      <div class="main">
            ...
            <Card :bordered="false" style="width:49%;height:410px;float:left;margin:0 auto;margin-top:50px;">
              <p slot="title">全校各个餐厅及其窗口的营业次数</p>
...
export default {
  ...
  methods: {
    // 从数据库获取总体统计数据
    getTotalData() {
      const path ='http://localhost:5000/group_consumption/total_data';
      axios({
        url: path,
        method: 'get'
      }).then((res) => {
    ...
    getTimesArray() {
      const path = 'http://localhost:5000/group_consumption/hallTimes';
      axios({
        url: path,
        method: 'get'
      }).then((res) => {
    ...
    getHallWindowDrill() {
      const path = 'http://localhost:5000/group_consumption/hallWindowDrill';
      axios({
        url: path,
        method: 'get'
      }).then((res) => {
    ...
    getAcademyGender() {
      const path = 'http://localhost:5000/group_consumption/academyGender';
      axios({
        url: path,
        method: 'get'
      }).then((res) => {
